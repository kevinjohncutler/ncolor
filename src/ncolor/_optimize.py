"""Label-equivariant 4-colouring optimisation.

The greedy WP picker in the core pipeline is locally optimal but commits
to colour choices without seeing global structure. On graphs with rings
or cycles, this can leave local minima where some colours go unused
within a region even though a valid 4-colouring using all 4 exists.

This module provides a Simulated-Annealing optimiser that:

  * operates on the per-label colour assignment (the "LUT" returned
    by ``ncolor.label(..., return_lut=True)``),
  * minimises a label-equivariant loss (no colormap/ΔE info enters),
...
Loss
----
The default loss penalises same-colour cell pairs at graph distance
exactly 2 ("2-hop"). On a cycle of length n ≥ 4 with 4 colours, this
loss is minimised by colourings whose every 3-cell window uses 3
distinct colours, which forces all 4 colours to appear. Equivalent to
the L(2,1)-labelling soft constraint with weight 1.
...
2-hop violations — but the optimiser still finds the lowest reachable
value, which empirically corresponds to "all 4 colours used everywhere
that they can be used".

Public surface
--------------
``optimize_two_hop(lut, adj, N, ...)`` — Kempe-SA on a 1-indexed LUT
  and adjacency dict. Returns ``(optimised_lut, final_loss)``.
...
def optimize_two_hop(lut_in, adj, N, n_colors=4,
    ...
    component, which is **guaranteed valid** by the four-colour theorem
    argument: the component is maximal in colours ``{X, Y}``, so every
    ...
    for _ in range(n_iters):
        ...
        if cu == 0:
            continue
        # Pick a Kempe partner colour ≠ cu.
...
def two_hop_loss(lut, adj, N):
    """Compute the 2-hop same-colour count for diagnostics."""
    pairs = _two_hop_pairs(adj, N)
