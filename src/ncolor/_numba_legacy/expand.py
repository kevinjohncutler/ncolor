"""
Euclidean Voronoi expand_labels — parallel separable parabolic-envelope algorithm.

Reference: Felzenszwalb & Huttenlocher (2012). Works for any ndim.
"""
import numpy as np
import numba
from numba import njit, prange


@njit(cache=True, parallel=True)
def _expand_batch_transpose(src_a, src_b, dst_a, dst_b):
    """Blocked parallel batched transpose: (A,B,C) → (A,C,B) for two arrays.
    Parallelises over all (a, b-tile, c-tile) combinations so both small-B and
    small-C cases distribute work across threads."""
    A, B, C = src_a.shape
    Bi = 64
    n_b = (B + Bi - 1) // Bi
    n_c = (C + Bi - 1) // Bi
    for i in prange(A * n_b * n_c):
        a  = i // (n_b * n_c)
        bc = i %  (n_b * n_c)
        b0 = (bc // n_c) * Bi
        c0 = (bc %  n_c) * Bi
        for b in range(b0, min(b0 + Bi, B)):
            for c in range(c0, min(c0 + Bi, C)):
                dst_a[a, c, b] = src_a[a, b, c]
                dst_b[a, c, b] = src_b[a, b, c]


@njit(cache=True, parallel=True, fastmath=True, boundscheck=False)
def _expand_l1_init_pass(h_lbl, h_dist):
    """First-axis L1 raster sweep: fused init from labels + forward + backward.
    Mirrors cpp_proto's chamfer_st_l1 phase 1.

    Tight inner loop: hoist row views once per slice (avoids 2D indexing in
    every iter), drop redundant int32 casts (numba infers from int32 array
    dtype), boundscheck off (we own the indices).
    """
    n_slices, N = h_lbl.shape
    INF = np.int32(1 << 29)
    for s in prange(n_slices):
        ll = h_lbl[s]
        dd = h_dist[s]
        # Forward (fused init)
        prev_d = INF
        prev_l = np.int32(0)
        for x in range(N):
            init_l = ll[x]
            init_d = np.int32(0) if init_l != 0 else INF
            cd = prev_d + 1
            if cd < init_d:
                dd[x] = cd
                ll[x] = prev_l
                prev_d = cd
            else:
                dd[x] = init_d
                prev_d = init_d
                prev_l = init_l
        # Backward
        for x in range(N - 2, -1, -1):
            cd = dd[x + 1] + 1
            if cd < dd[x]:
                dd[x] = cd
                ll[x] = ll[x + 1]


@njit(cache=True, parallel=True, fastmath=True, boundscheck=False)
def _expand_l1_axis_propagate(arr_lbl, arr_dist, n_bands_c):
    """Generic ND L1 propagate sweep along an axis, no transpose.

    Caller reshapes the input to (A, B, C) where:
        A = product of axes BEFORE the sweep axis      (outer parallel)
        B = length of the sweep axis                   (serial: forward+backward)
        C = product of axes AFTER the sweep axis        (inner contiguous, vectorisable)

    Parallel domain is `A * nb` where `nb` is derived from `n_bands_c` so
    smaller A workloads still expose enough parallel iters to fill all
    threads. Mirrors cpp_proto's `chamfer_st_l1_nd` stride-aware structure.
    """
    A, B, C = arr_lbl.shape
    band_w = max(1, (C + n_bands_c - 1) // n_bands_c)
    nb = (C + band_w - 1) // band_w
    for idx in prange(A * nb):
        a = idx // nb
        bnd = idx % nb
        c0 = bnd * band_w
        c1 = min(C, c0 + band_w)
        for b in range(1, B):
            for c in range(c0, c1):
                cd = arr_dist[a, b - 1, c] + 1
                if cd < arr_dist[a, b, c]:
                    arr_dist[a, b, c] = cd
                    arr_lbl[a, b, c]  = arr_lbl[a, b - 1, c]
        for b in range(B - 2, -1, -1):
            for c in range(c0, c1):
                cd = arr_dist[a, b + 1, c] + 1
                if cd < arr_dist[a, b, c]:
                    arr_dist[a, b, c] = cd
                    arr_lbl[a, b, c]  = arr_lbl[a, b + 1, c]


@njit(cache=True, parallel=True, fastmath=True, boundscheck=False)
def _expand_l1_propagate_pass(h_lbl, h_dist):
    """Subsequent-axis L1 sweep: forward + backward propagate from existing dist.
    No init step (dist starts with prior-axis distances).
    """
    n_slices, N = h_lbl.shape
    for s in prange(n_slices):
        ll = h_lbl[s]
        dd = h_dist[s]
        for x in range(1, N):
            cd = dd[x - 1] + 1
            if cd < dd[x]:
                dd[x] = cd
                ll[x] = ll[x - 1]
        for x in range(N - 2, -1, -1):
            cd = dd[x + 1] + 1
            if cd < dd[x]:
                dd[x] = cd
                ll[x] = ll[x + 1]


@njit(cache=True, parallel=True, fastmath=True)
def _expand_envelope_pass(h_lbl, h_dist):
    """
    Parabolic lower-envelope pass for one axis (Felzenszwalb & Huttenlocher 2012).
    h_lbl:  (n_slices, N) int32 — 0 = no seed; updated in-place to nearest label.
    h_dist: (n_slices, N) int32 — accumulated squared distance; updated in-place.

    ``fastmath=True`` lets LLVM apply reassociation and reciprocal-multiply
    optimizations on the parabolic-intersection arithmetic — verified safe
    here because all FP values are well within double precision (label
    coordinates ≤ image dimension, squared distances ≤ image diagonal²).
    """
    n_slices, N = h_lbl.shape
    for s in prange(n_slices):
        v   = np.empty(N, dtype=np.int32)
        lbl = np.empty(N, dtype=np.int32)
        g   = np.empty(N, dtype=np.int32)
        z   = np.empty(N + 1, dtype=np.float64)
        k   = np.int32(0)
        for i in range(N):
            if h_lbl[s, i] == 0:
                continue
            gi  = np.int32(h_dist[s, i])
            fi  = np.float64(i)
            gf  = np.float64(gi)
            new_z = np.float64(-1e18)
            while k > 0:
                top = k - 1
                ft  = np.float64(v[top])
                sv  = (gf + fi*fi - np.float64(g[top]) - ft*ft) / (2.0 * (fi - ft))
                if sv > z[top]:
                    new_z = sv
                    break
                k -= np.int32(1)
            z[k] = new_z; v[k] = np.int32(i); lbl[k] = h_lbl[s, i]; g[k] = gi
            k += np.int32(1)
        if k == 0:
            continue
        j = np.int32(0)
        for i in range(N):
            while j + 1 < k and np.float64(i) >= z[j + 1]:
                j += np.int32(1)
            di = np.int32(i) - v[j]
            h_lbl[s, i]  = lbl[j]
            h_dist[s, i] = g[j] + di * di


_expand_buf_shape = (0,)
_expand_buf_dtype = None
_expand_h_lbl   = _expand_h_dist = _expand_h_lbl_T = _expand_h_dist_T = _expand_out = None

def _ensure_expand_bufs(shape, dtype):
    global _expand_buf_shape, _expand_buf_dtype
    global _expand_h_lbl, _expand_h_dist, _expand_h_lbl_T, _expand_h_dist_T, _expand_out
    if shape == _expand_buf_shape and dtype == _expand_buf_dtype:
        return
    _expand_h_lbl  = np.empty(shape, dtype=np.int32)
    _expand_h_dist = np.empty(shape, dtype=np.int32)
    _expand_out    = np.empty(shape, dtype=dtype)
    if len(shape) >= 2:
        _expand_h_lbl_T  = np.empty(shape, dtype=np.int32)
        _expand_h_dist_T = np.empty(shape, dtype=np.int32)
    _expand_buf_shape = shape
    _expand_buf_dtype = dtype


def expand_labels(label_image, metric='l2'):
    """
    Expand labels into every background pixel using nearest-label Voronoi
    assignment in either L2 (Euclidean) or L1 (Manhattan) metric.

    Parameters
    ----------
    label_image : ndarray, integer
        Input; nonzero pixels are seeds whose values are labels.
    metric : {'l2', 'l1'}
        - 'l2' (default): parabolic-envelope algorithm (Felzenszwalb &
          Huttenlocher 2012). Exact Euclidean.
        - 'l1': Saito-Toriwaki separable raster sweep (forward+backward
          1D L1 transform per axis). Exact Manhattan distance.
        Both run in Numba, parallel over slices, ndim-generic.
    """
    _ensure_expand_bufs(label_image.shape, label_image.dtype)
    ndim = label_image.ndim
    np.copyto(_expand_h_lbl, label_image, casting='unsafe')
    _expand_h_dist[:] = 0

    use_l1 = (metric == 'l1')

    n_threads = numba.config.NUMBA_NUM_THREADS

    for ax in range(ndim - 1, -1, -1):
        n = label_image.shape[ax]
        first = (ax == ndim - 1)
        if first:
            # Innermost axis: contiguous 1D sweep, no transpose either way.
            if use_l1:
                _expand_l1_init_pass(_expand_h_lbl.reshape(-1, n),
                                     _expand_h_dist.reshape(-1, n))
            else:
                _expand_envelope_pass(_expand_h_lbl.reshape(-1, n),
                                      _expand_h_dist.reshape(-1, n))
        else:
            if use_l1:
                # ND L1 fast path: sweep along axis `ax` without transpose.
                # Reshape view as (A, B, C); the kernel parallelises over
                # A*bands_of_C and the inner C-loop is auto-vectorisable.
                A = int(np.prod(label_image.shape[:ax])) if ax > 0 else 1
                C = int(np.prod(label_image.shape[ax + 1:]))
                hl = _expand_h_lbl.reshape(A, n, C)
                hd = _expand_h_dist.reshape(A, n, C)
                # Want enough parallel iters to fill all threads.
                n_bands_c = max(1, (n_threads + A - 1) // A)
                _expand_l1_axis_propagate(hl, hd, n_bands_c)
            else:
                # L2 still uses transpose round-trip — its inner-loop kernel
                # has a serial envelope-construction dependency that won't
                # vectorise well in stride-aware form.
                A = int(np.prod(label_image.shape[:ax])) if ax > 0 else 1
                C = int(np.prod(label_image.shape[ax + 1:]))
                hl = _expand_h_lbl.reshape(A, n, C)
                hd = _expand_h_dist.reshape(A, n, C)
                tl = _expand_h_lbl_T.reshape(A, C, n)
                td = _expand_h_dist_T.reshape(A, C, n)
                _expand_batch_transpose(hl, hd, tl, td)
                _expand_envelope_pass(tl.reshape(-1, n), td.reshape(-1, n))
                _expand_batch_transpose(tl, td, hl, hd)

    np.copyto(_expand_out, _expand_h_lbl, casting='unsafe')
    return _expand_out
